...
$borrowing = "UPDATE books SET status = 'Borrowed', quantity = '$balance' WHERE md5(id) = '$id'";
$results = $conn->query($borrowing);

if($results == true){
	echo "<div style='background:green;padding:7px;color:white'>Book borrowed successfully</div>";
}else{
		echo "<div style='background:red;padding:7px;color:white'>Book borrowed successfully</div>";
	
}

$borrow = "INSERT INTO borrowed (user,title,author,isbn,bookfile,bookimage,status) VALUES ('$user','$title','$author','$isbn','$bookfile','$bookimage','$status')";
